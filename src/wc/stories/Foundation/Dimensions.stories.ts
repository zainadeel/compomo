...
export const Radii: Story = {
  render: () => html`
    <div style=${pageStyle}>
      ...
      <div
        style="display:grid;grid-template-columns:repeat(auto-fill,minmax(var(--dimension-size-800),1fr));gap:var(--dimension-space-200)"
      >
        ${RADIUS_TOKENS.map(
          token => html`
            <div
  ...
  render: () => html`
    <div style=${pageStyle}>
      ...
      <div style="display:flex;flex-wrap:wrap;gap:var(--dimension-space-100)">
        ${['base', 'raised', 'overlay', 'modal', 'floating', 'tooltip'].map(
          token => html`
            <div
              style="
