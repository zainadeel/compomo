...
function CategoryColors(): TemplateResult {
  return html` <div style="${PAGE}">
    ${section(
      'Category (12)',
      'Categorical palette for charts and graphs',
      html` <div style="${GRID} grid-template-columns: repeat(auto-fill, minmax(80px, 1fr));">
                ...
                ${Array.from({ length: steps }, (_, i) => i + 1).map(
                  n =>
                    html` <div style="flex: 1;">
                      ${swatch(`color-data-sequence-blue-${steps}-${n}`, `${n}`)}
                    </div>`
                )}
              </div>
            </div>`
...
function DivergingColors(): TemplateResult {
  return html` <div style="${PAGE}">
    ${section(
      ...
      '5, 7, and 9-step diverging palettes',
      html`
        ${[5, 7, 9].map(
          steps =>
            html` <div style="margin-bottom: 12px;">
              ...
              <div style="display: flex; gap: 4px;">
                ${Array.from({ length: steps }, (_, i) => i + 1).map(
                  n =>
                    html` <div style="flex: 1;">
                      ${swatch(`color-data-diverging-blue-orange-${steps}-${n}`, `${n}`)}
                    </div>`
                )}
              </div>
            </div>`
...
function WinLossColors(): TemplateResult {
  return html` <div style="${PAGE}">
    ${section(
      'Win/Loss',
      undefined,
      html` <div style="display: flex; gap: 8px; max-width: 400px;">
...
function MiscColors(): TemplateResult {
  ...
  return html` <div style="${PAGE}">
    ${section(
      'Misc',
      'Semantic intensity steps (strong → faint)',
      html` <div style="display: flex; gap: 8px; max-width: 400px;">
        ${steps.map(
          step => html` <div style="flex: 1;">${swatch(`color-data-misc-${step}`, step)}</div>`
        )}
      </div>`
    )}
  </div>`;
...
function IntentColors(): TemplateResult {
  ...
  return html` <div style="${PAGE}">
    ${section(
      'Intent',
      'Semantic intent colors for data visualization',
      html` <div style="${GRID}">
...
function StatusDeviceColors(): TemplateResult {
  ...
  return html` <div style="${PAGE}">
    ${section(
      'Status Device',
      'Device status colors for fleet / hardware charts',
      html` <div style="${GRID}">
  ...
  return html` <div style="${PAGE}">
    ${section(
      ...
      html` <div style="display: flex; gap: 8px; max-width: 400px;">
        ${scores.map(
          score =>
            html` <div style="flex: 1;">${swatch(`color-data-score-safety-${score}`, score)}</div>`
        )}
      </div>`
    )}
  </div>`;
