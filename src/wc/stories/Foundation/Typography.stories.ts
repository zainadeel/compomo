...
const PAGE =
  'font-family: var(--typography-font-family-ui); padding: var(--dimension-space-400); display: flex; flex-direction: column; gap: calc(var(--dimension-space-100) * 5); background: var(--color-background-primary); color: var(--color-foreground-primary); min-height: 100vh; box-sizing: border-box;';
const SECTION = 'display: flex; flex-direction: column; gap: var(--dimension-space-050);';
const H2 =
  'font-size: var(--typography-fontsize-lg); font-weight: var(--typography-weight-semibold); color: var(--color-foreground-primary); margin: 0 0 var(--dimension-space-200); letter-spacing: var(--typography-letterspacing-negative-half); font-family: var(--typography-font-family-ui);';
const ROW =
  'display: flex; align-items: baseline; gap: var(--dimension-space-200); padding: var(--dimension-space-125) 0; border-bottom: var(--dimension-stroke-width-012) solid var(--color-border-tertiary);';
const LBL =
  'font-size: var(--typography-fontsize-xs); color: var(--color-foreground-secondary); min-width: 220px; flex-shrink: 0; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; padding-top: var(--dimension-space-025);';
const SPEC =
  'font-size: var(--typography-fontsize-xs); color: var(--color-foreground-tertiary); min-width: 140px; flex-shrink: 0; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;';
...
function VariantsStory(): TemplateResult {
  return html`
    <div style="${PAGE}">
      ...
      <div style="${SECTION}">
        ${TYPOGRAPHY_STYLE_ROWS.map(
          row => html`
            <div style="${ROW}">
              <span style="${LBL}">${row.label}</span>
              <span style="${SPEC}">${formatTypographySpec(row)}</span>
              <ds-text variant=${row.variant} .emphasis=${row.emphasis} color="primary" as="span"
                >${row.sample}</ds-text
              >
            </div>
          `
...
function StylesStory(): TemplateResult {
  return html`
    <div style="${PAGE}">
      ...
      <div style="${SECTION}">
        <div style="${ROW}">
          ...
          <ds-text variant="text-body-medium" color="primary" style="flex:1; max-width:460px;">
            No truncation — The quick brown fox jumps over the lazy dog. Sphinx of black quartz,
        ...
        ${([1, 2, 3] as const).map(
          n => html`
            <div style="${ROW}">
              ...
              >
                Truncated to ${n} line${n > 1 ? 's' : ''} — The quick brown fox jumps over the lazy
                dog. Sphinx of black quartz, judge my vow. How vexingly quick daft zebras jump. Pack
      ...
      <div style="${SECTION}">
        ${(['wrap', 'balance', 'pretty'] as const).map(
          w => html`
            <div style="${ROW}">
              ...
              >
                Wrap mode: ${w}. The quick brown fox jumps over the lazy dog and the fence beyond
                the meadow.
              </ds-text>
            </div>
          `
