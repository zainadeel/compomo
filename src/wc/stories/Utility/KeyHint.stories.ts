...
const keyHint = (shortcut: string, size: (typeof SIZES)[number]) => html`
  <span class=${`key-hint-group ds-control--${size}`} aria-label=${`${shortcut} shortcut`}>
    ${shortcutKeyLabels(shortcut).map(
      label => html`
        <span
          class=${`key-hint${Array.from(label).length > 1 ? ' key-hint--wide' : ''}`}
          aria-hidden="true"
        >
  ...
  render: () => html`
    ...
    >
      ${SIZES.map(
        size => html`
          <div
            style="display: flex; flex-wrap: wrap; align-items: center; gap: var(--dimension-space-100);"
          >
            ${keyHint('K', size)} ${keyHint('⌘S', size)} ${keyHint('⌘⇧S', size)}
            ${keyHint('⌃⇧K', size)}
          </div>
        `
      )}
