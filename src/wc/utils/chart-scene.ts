...
import { chartIntentColor, defaultChartTheme, type ChartTheme } from './chart-theme';
...
const EMPTY_AXIS: ChartSceneAxis = {
  ticks: [],
  grid: false,
  rotate: 0,
  line: false,
...
function developmentWarning(message: string): void {
  ...
  if (typeof console !== 'undefined' && (browserDevelopment || nodeDevelopment))
    console.warn(message);
}

function positionalKey(
...
function inferDatumKey(
  ...
  developmentWarning(
    `[ds-chart] ${observation.markId} is using row index ${observation.index} as its datum key. Author a key channel for stable focus.`
  );
...
function observationsForMark(mark: ChartMark, markIndex: number): Observation[] {
  ...
  const ids = bases
    .map(base =>
      base.datum && typeof base.datum === 'object' ? (base.datum as { id?: unknown }).id : undefined
    )
    .filter((id): id is ChartKey => typeof id === 'string' || typeof id === 'number');
  const uniqueIds = uniqueValues(ids);
  const uniquePositions = uniqueValues(
    bases.map(positionalKey).filter((key): key is string => key !== undefined)
  );
...
function configuredScale(source: ChartAxisOptions['scale']): {
  scale: ChartScale;
  configured: boolean;
} {
  const candidate = source as ChartScale;
  if (typeof candidate.domain === 'function' && typeof candidate.range === 'function')
    return { scale: candidate.copy?.() ?? candidate, configured: true };
  const scale = (source as () => unknown)() as ChartScale;
  if (!scale || typeof scale.domain !== 'function' || typeof scale.range !== 'function')
    throw new TypeError(
      'Chart scales must be native D3 scale factories or configured scale instances.'
    );
...
function resolveScale(
  ...
  const domain =
    resolved.configured && existingDomain.length > 0
      ? existingDomain
      : inferDomain(values, includeZero);
...
function tickValues(
  ...
  const count =
    axis.axis === false ? 0 : (axis.axis?.ticks?.count ?? Math.max(2, Math.floor(width / 80)));
...
function thinTicks(
  ...
  if (!kept.some(tick => tick.key === last.key)) {
    while (kept.length > 1) {
      ...
      const candidateSize = horizontal
        ? measurer(candidate.label).width
        : measurer(candidate.label).height;
...
function sceneAxis(
  ...
  return {
    ...
    ticks: thinTicks(
      ...
      horizontal,
      measurer,
      axis.axis?.ticks?.spacing ?? DEFAULT_TICK_SPACING
    ),
...
function solveCartesianMargins(
  ...
  let left = Math.max(
    ...
    yAxis.ticks.length > 0
      ? yAxis.tickSize + yAxis.tickPadding + widestY + (yAxis.label ? 18 : 0)
      : 0
  );
  ...
  let bottom = Math.max(
    requested.bottom,
    xAxis.ticks.length > 0
      ? xAxis.tickSize + xAxis.tickPadding + tallestX + (xAxis.label ? 18 : 0)
      : 0
  );
...
function styleFor(
  ...
  return {
    ...
    strokeWidth: visualValue(
      ...
      source,
      options.strokeWidth,
      defaults.strokeWidth ?? defaultChartTheme.seriesStrokeWidth
    ),
...
function colorResolver(spec: ChartSpec, observations: Observation[][]) {
  const inferred = distinct(
    observations.flatMap(group =>
      group.map(observation => observation.color ?? observation.z).filter(validValue)
    )
  );
  ...
  const map = new Map(
    domain.map((value, index) => [
      chartValueKey(value),
      range[index % Math.max(1, range.length)] ?? categoryColor(index),
    ])
  );
  return (observation: Observation): string => {
    const value = observation.color ?? observation.z;
    return value === undefined
      ? (range[0] ?? categoryColor(0))
      : (map.get(chartValueKey(value)) ?? categoryColor(0));
  };
}
  ...
  marks.forEach((mark, markIndex) => {
      ...
      groups.forEach(group => {
        ...
        if (mark.kind === 'line-y') {
          const line = lineShape<Observation>()
            .defined(
              observation =>
                ...
                scalePosition(xScale, observation.x) !== undefined &&
                scalePosition(yScale, observation.y) !== undefined
            )
          ...
          if (d)
            nodes.push({
              type: 'path',
              key: `${first.markId}:series:${chartValueKey(first.z ?? first.color ?? 'series')}`,
              markId: first.markId,
              d,
              style: styleFor(first, mark, color, {
                fill: 'none',
                stroke: color,
    ...
    if (mark.kind === 'bar-y' || mark.kind === 'bar-x') {
      ...
      const categories = distinct(
        observations
          .map(observation => (vertical ? observation.x : observation.y))
          .filter(validValue)
      );
      const categoryBand =
        categoryScale.scale.bandwidth?.() ??
        Math.max(
          1,
          ((vertical ? plot.width : plot.height) / Math.max(1, categories.length)) * 0.75
        );
      const seriesValues = distinct(
        observations.map(observation => observation.z).filter(validValue)
      );
      const groupedScale = scaleBand<ChartValue>()
        .domain(seriesValues.length > 0 ? seriesValues : ['series'])
        .range([0, categoryBand])
        .padding(0.08);
      ...
      observations.forEach(observation => {
        ...
        const zeroGap =
          layout === 'stacked' && signs?.positive && signs.negative ? theme.stackGap / 2 : 0;
        ...
        if (mark.options.interactive !== false)
          points.push(
            pointFor(
              observation,
              vertical ? bandStart + bandSize / 2 : endPosition,
              vertical ? endPosition : bandStart + bandSize / 2,
              color
            )
          );
      });
    ...
    if (mark.kind === 'box-y') {
      ...
      byCategory.forEach(group => {
        ...
        const statistics = boxStatistics(
          group.map(observation => observation.y as number),
          boxOptions.whisker ?? 1.5
        );
        const center = scalePosition(xScale, first.x);
        if (!statistics || center === undefined) return;
        const categoryBand =
          xScale.scale.bandwidth?.() ??
          Math.max(24, (plot.width / Math.max(1, byCategory.size)) * 0.6);
        const boxWidth =
          boxOptions.inset === undefined
        ...
        const lineStyle = styleFor(first, mark, color, {
          fill: 'none',
          stroke: color,
          strokeWidth: theme.seriesStrokeWidth,
        });
        const boxNode = rectNode(
          first,
          mark,
          color,
          left,
          ...
          yQ3 as number,
          yQ1 as number,
          boxOptions.radius ?? theme.barRadius
        );
        boxNode.style.fillOpacity = visualValue(
          first.datum,
          first.index,
          first.source,
          boxOptions.fillOpacity,
          theme.boxFillOpacity
        );
        nodes.push(
          {
            type: 'line',
            key: `${first.sceneKey}:whisker`,
            markId: first.markId,
            x1: center,
            x2: center,
            y1: yUpper as number,
          ...
          {
            type: 'line',
            key: `${first.sceneKey}:median`,
            markId: first.markId,
            x1: left,
            x2: right,
            y1: yMedian as number,
            y2: yMedian as number,
            style: lineStyle,
          }
        );
        statistics.outliers.forEach((value, index) => {
          ...
          if (y !== undefined)
            nodes.push({
              type: 'circle',
              key: `${first.sceneKey}:outlier:${index}`,
              markId: first.markId,
              x: center,
              y,
              radius: dotCircleRadius(theme.dotRadius, theme.dotHaloWidth),
              clipToPlot: false,
              style: styleFor(first, mark, color, {
                fill: 'var(--color-background-primary)',
                stroke: color,
                strokeWidth: theme.dotHaloWidth,
              }),
            });
        });
        const summaryObservation: Observation = {
          ...
          datumKey: chartValueKey(first.x),
          sceneKey: `${first.markId}:${chartValueKey(first.x)}`,
        };
        if (mark.options.interactive !== false)
          points.push(pointFor(summaryObservation, center, yMedian as number, color));
      });
      return;
    }
    ...
    if (mark.kind === 'rect') {
      ...
      observations.forEach(observation => {
        if (![observation.x1, observation.x2, observation.y1, observation.y2].every(finiteNumber))
          return;
        ...
        if (mark.options.interactive !== false)
          points.push(
            pointFor(
              ...
              Math.min(y1 as number, y2 as number),
              color,
              {
                xValue: ((observation.x1 as number) + (observation.x2 as number)) / 2,
                yValue: observation.y2 as number,
              }
            )
          );
      });
    ...
    if (mark.kind === 'cell') {
      ...
      observations.forEach(observation => {
        if (
          observation.x === undefined ||
          observation.y === undefined ||
          !finiteNumber(observation.intensity)
        )
          return;
        const x = scalePosition(xScale, observation.x);
        const y = scalePosition(yScale, observation.y);
        ...
        const width =
          xScale.scale.bandwidth?.() ??
          plot.width /
            Math.max(1, distinct(observations.map(item => item.x).filter(validValue)).length);
        const height =
          yScale.scale.bandwidth?.() ??
          plot.height /
            Math.max(1, distinct(observations.map(item => item.y).filter(validValue)).length);
        ...
        const color = visualValue(
          ...
          observation.source,
          options.fill,
          chartIntentColor(options.intent)
        );
        const ratio =
          maximum === minimum ? 1 : (observation.intensity - minimum) / (maximum - minimum);
        ...
        node.style.fillOpacity = visualValue(
          observation.datum,
          observation.index,
          observation.source,
          options.fillOpacity,
          minOpacity + ratio * (maxOpacity - minOpacity)
        );
        nodes.push(node);
        if (mark.options.interactive !== false)
          points.push(pointFor(observation, x, y, color, { value: observation.intensity }));
      });
      return;
    }
    ...
    observations.forEach(observation => {
      const color = colorFor(observation);
      if (
        (mark.kind === 'rule-x' || mark.kind === 'band-x') &&
        (observation.x !== undefined || finiteNumber(observation.x1))
      ) {
        ...
        if (mark.kind === 'rule-x')
          nodes.push({
            ...
            y1: plot.top,
            y2: plot.bottom,
            style: styleFor(observation, mark, 'var(--color-foreground-tertiary)', {
              stroke: 'var(--color-foreground-tertiary)',
              fill: 'none',
              strokeWidth: theme.ruleStrokeWidth,
            }),
          });
        else {
          const node = rectNode(
          ...
          node.style.fillOpacity = visualValue(
            ...
            observation.index,
            observation.source,
            mark.options.fillOpacity,
            theme.annotationFillOpacity
          );
          nodes.push(node);
        }
        return;
      ...
      if (
        (mark.kind === 'rule-y' || mark.kind === 'band-y') &&
        (observation.y !== undefined || finiteNumber(observation.y1))
      ) {
      ...
      if (mark.kind === 'dot') {
        ...
        nodes.push({
          ...
          x,
          y,
          radius: dotCircleRadius(radius, theme.dotHaloWidth),
          clipToPlot: false,
          style: styleFor(observation, mark, color, {
            fill: color,
        ...
        nodes.push({
          ...
          dx:
            'dx' in options
              ? visualValue(observation.datum, observation.index, observation.source, options.dx, 0)
              : 0,
          dy:
            'dy' in options
              ? visualValue(observation.datum, observation.index, observation.source, options.dy, 0)
              : 0,
          style: {
            ...styleFor(observation, mark, 'var(--color-foreground-primary)', {
...
export function resolveDonutPadAngle(
  gap: number,
  innerRadius: number,
  outerRadius: number
): number {
...
function buildPolarScene(
  ...
  ChartScene,
  'plot' | 'guides' | 'nodes' | 'points' | 'focusDotPointKeys' | 'arcHitRegions'
> {
  const coordinate = spec.coordinate as ChartPolarCoordinate;
  const focusDotPointKeys = new Set<string>();
  const arcHitRegions: ChartArcHitRegion[] = [];
  const angleDomain = distinct([
    ...(spec.angle?.domain ?? []),
    ...observations.flatMap(group =>
      group.map(observation => observation.angle).filter(validValue)
    ),
  ]);
  const angleFor = (value: ChartValue): number => {
    const index = Math.max(
      0,
      angleDomain.findIndex(candidate => chartValueKey(candidate) === chartValueKey(value))
    );
  ...
  if ((coordinate.grid ?? 'polygon') === 'polygon') {
    for (let pass = 0; pass < 8; pass += 1) {
      ...
      angleDomain.forEach(angleValue => {
        ...
        const measurement = measurer(
          spec.angle?.ticks?.format?.(angleValue, locale) ?? formatTick(angleValue, locale)
        );
        const left =
          localX < -0.01
            ? position.x - measurement.width
            : localX > 0.01
              ? position.x
  ...
  const radiusValues = observations.flatMap(group =>
    group.flatMap(observation =>
      [observation.radius, observation.radius1, observation.radius2].filter(finiteNumber)
    )
  );
  ...
  if ((coordinate.grid ?? 'polygon') === 'polygon' && angleDomain.length >= 3) {
    ...
    tickValuesForRadius
      ...
      .forEach((value, ringIndex) => {
        const ringRadius = scalePosition(radiusScale, value);
        if (ringRadius === undefined) return;
        const polygon = [...angleDomain, angleDomain[0]].map(angle =>
          polarPoint(centerX, centerY, angleFor(angle), ringRadius)
        );
        const d = lineShape<{ x: number; y: number }>()
          .x(point => point.x)
          .y(point => point.y)(polygon);
        if (d)
          guides.push({
            type: 'path',
            key: `polar-ring:${ringIndex}`,
            markId: 'polar-guide',
            d,
            style: {
              fill: 'none',
    ...
    angleDomain.forEach((angleValue, index) => {
      ...
      guides.push(
        {
          type: 'line',
          key: `polar-spoke:${index}`,
          markId: 'polar-guide',
          className: 'chart__grid chart__polar-guide',
          x1: centerX,
          y1: centerY,
          x2: end.x,
          y2: end.y,
        ...
        {
          type: 'text',
          key: `polar-label:${index}`,
          markId: 'polar-guide',
          className: 'chart__tick chart__polar-label',
          measure: true,
          x: label.x,
          y: label.y,
          text: spec.angle?.ticks?.format?.(angleValue, locale) ?? formatTick(angleValue, locale),
          dominantBaseline: localY < -0.01 ? 'auto' : localY > 0.01 ? 'hanging' : 'middle',
          style: {
            fill: 'var(--color-foreground-secondary)',
            stroke: 'none',
            textAnchor: localX < -0.01 ? 'end' : localX > 0.01 ? 'start' : 'middle',
            fontFamily: 'var(--typography-font-family-ui)',
  ...
  marks.forEach((mark, markIndex) => {
    ...
    if (mark.kind === 'arc') {
      markObservations.forEach(observation => {
        ...
        const options = mark.options;
        const inner =
          'innerRadius' in options
            ? visualValue(
                observation.datum,
                observation.index,
                observation.source,
                options.innerRadius,
                defaultInnerRadius / Math.max(1, outerRadius)
              ) * outerRadius
            : defaultInnerRadius;
        const outer =
          'outerRadius' in options
            ? visualValue(
                observation.datum,
                observation.index,
                observation.source,
            ...
            : outerRadius;
        const corner =
          'cornerRadius' in options
            ? visualValue(
                observation.datum,
                observation.index,
                observation.source,
                options.cornerRadius,
                theme.polarCornerRadius
              )
            : theme.polarCornerRadius;
        const defaultPadAngle =
          markObservations.length > 1 && inner > 0
        ...
        const generator = arcShape<{ startAngle: number; endAngle: number }>()
          ...
          .padAngle(padAngle);
        const arcDatum = {
          startAngle: polarAngle(coordinate, observation.theta1),
          endAngle: polarAngle(coordinate, observation.theta2),
        };
        ...
        if (mark.options.interactive !== false)
          points.push(
            pointFor(observation, centerX + localX, centerY + localY, color, {
              xValue: observation.label ?? observation.datumKey,
              yValue: observation.value ?? observation.theta2 - observation.theta1,
              value: observation.value ?? observation.theta2 - observation.theta1,
            })
          );
      });
      return;
    ...
    if (mark.kind === 'radial-line' || mark.kind === 'radial-area') {
      groupObservations(markObservations).forEach(group => {
        ...
        const valid = group.filter(
          observation => observation.angle !== undefined && finiteNumber(observation.radius)
        );
        if (valid.length === 0) return;
        const color = colorFor(first);
        if (mark.kind === 'radial-line') {
          const generator = lineRadial<Observation>()
            .angle(observation => angleFor(observation.angle as ChartValue))
            .radius(observation => scalePosition(radiusScale, observation.radius as number) ?? 0)
            .curve(
              'curve' in mark.options && mark.options.curve ? mark.options.curve : curveLinearClosed
            );
          ...
          const generator = areaRadial<Observation>()
            .angle(observation => angleFor(observation.angle as ChartValue))
            .innerRadius(observation => scalePosition(radiusScale, observation.radius1 ?? 0) ?? 0)
            .outerRadius(
              observation =>
                scalePosition(radiusScale, observation.radius2 ?? observation.radius ?? 0) ?? 0
            )
            .curve(
              'curve' in mark.options && mark.options.curve ? mark.options.curve : curveLinearClosed
            );
  ...
  const arcMarkIds = new Set(
    marks
      .map((mark, index) =>
        mark.kind === 'arc' ? (mark.options.id ?? `${mark.kind}-${index}`) : undefined
      )
...
export function compileChartScene(
  ...
  if (polar) {
    ...
    return {
      ...
      yAxis: { ...EMPTY_AXIS },
      center: spec.center,
      focus: spec.focus ?? 'nearest',
      maxFocusDistance: spec.maxFocusDistance,
      tooltip: spec.tooltip,
      clip: spec.clip ?? true,
      focusDotCircleRadius: dotCircleRadius(theme.focusDotRadius, theme.dotHaloWidth),
      focusDotHaloWidth: theme.dotHaloWidth,
    };
  }

  if (!spec.x || !spec.y)
    throw new TypeError('Cartesian chart definitions require x and y scale options.');
  ...
  for (let pass = 0; pass < 4; pass += 1) {
    const nextMargin = solveCartesianMargins(
      width,
      height,
      requestedMargin,
      xAxis,
    ...
    const settled = (['top', 'right', 'bottom', 'left'] as const).every(
      side => Math.abs(nextMargin[side] - margin[side]) <= 0.25
    );
    margin = nextMargin;
    plot = plotFor(width, height, margin);
  ...
  return {
    ...
    nodes: built.nodes,
    points: built.points,
    focusDotPointKeys: built.focusDotPointKeys,
    arcHitRegions: [],
    center: spec.center,
    focus: spec.focus ?? 'nearest',
    maxFocusDistance: spec.maxFocusDistance,
...
export function findChartFocus(
  ...
  const primary = points.reduce((nearest, point) =>
    distance(point) < distance(nearest) ? point : nearest
  );
  if (distance(primary) > maxDistance) return undefined;
  if (mode === 'group-x')
    return {
      primary,
      points: points.filter(point => chartValueKey(point.xValue) === chartValueKey(primary.xValue)),
    };
  if (mode === 'group-y')
    return {
      primary,
      points: points.filter(point => chartValueKey(point.yValue) === chartValueKey(primary.yValue)),
    };
