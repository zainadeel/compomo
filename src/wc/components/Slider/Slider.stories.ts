...
const STACK =
  'display:flex;flex-direction:column;gap:var(--dimension-space-300);width:min(100%,var(--dimension-form-width-md));';
const REVIEW_GRID =
  'display:grid;grid-template-columns:repeat(auto-fit,minmax(var(--dimension-form-width-xs),1fr));gap:var(--dimension-space-300);align-items:start;width:min(100%,var(--dimension-modal-width-2xl));';
const PANEL =
  'display:flex;flex-direction:column;gap:var(--dimension-space-150);padding:var(--dimension-space-200);border:var(--dimension-stroke-width-012) solid var(--color-border-tertiary);border-radius:var(--dimension-radius-050);';
  ...
  render: args => html`
    <div
      style=${args['orientation'] === 'vertical'
        ? 'height:320px;'
        : 'width:min(100%,var(--dimension-form-width-md));'}
    >
      <ds-slider
    ...
    <div style=${REVIEW_GRID}>
      ...
      <div style=${PANEL}>
        ...
        <ds-slider
          value="65"
          .showValue=${false}
          aria-label="Volume without a visible label"
        ></ds-slider>
      </div>
    </div>
  `,
  ...
  render: () => html`
    <div style=${STACK}>
      ...
      <ds-slider
        ...
        label="Budget range"
        value-prefix="$"
        start-label="Minimum budget"
        end-label="Maximum budget"
      ></ds-slider>
...
export const Events: Story = {
  ...
  render: () => {
    const update = (kind: 'change' | 'commit', event: CustomEvent<number | number[]>) => {
      const output = (event.currentTarget as HTMLElement).parentElement?.querySelector<HTMLElement>(
        `[data-${kind}]`
      );
    ...
    return html`
      <div style=${STACK}>
        ...
        ></ds-slider>
        <div
          style="display:grid;grid-template-columns:max-content 1fr;gap:var(--dimension-space-050) var(--dimension-space-100);"
        >
          <ds-text as="span" variant="text-body-small" color="secondary">Continuous</ds-text>
          <ds-text as="span" variant="text-body-small" font-feature="tabular-nums" data-change
            >—</ds-text
          >
          <ds-text as="span" variant="text-body-small" color="secondary">Committed</ds-text>
          <ds-text as="span" variant="text-body-small" font-feature="tabular-nums" data-commit
            >—</ds-text
          >
        </div>
      </div>
  ...
  render: () => {
    ...
    return html`
      <form style=${STACK} @submit=${submit}>
        ...
        <ds-text as="span" variant="text-body-small" font-feature="tabular-nums" data-form-output
          >Submit to inspect values.</ds-text
        >
      </form>
    `;
  },
