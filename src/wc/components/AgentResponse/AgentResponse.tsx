...
export class AgentResponse {
  ...
  private renderPart(part: AgentResponsePart) {
    switch (part.type) {
      case 'markdown':
        return <ds-markdown key={part.id} content={part.content} streaming={part.state === 'streaming'} />;
      case 'activity':
        return <ds-agent-activity key={part.id} items={part.items} />;
      case 'tool':
        return <ds-agent-tool-call key={part.id} name={part.name} label={part.label} state={part.state} input={part.input} output={part.output} error={part.error} />;
      case 'attachments':
  ...
  render() {
    return (
      <Host>
        <ds-message messageId={this.messageId} direction="incoming" author={this.author} timestamp={this.timestamp} streaming={this.streaming}>
