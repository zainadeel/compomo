...
test('treats kind action as a locked customizer column', () => {
...
test('refuses to hide action columns or the last visible data column', () => {
  ...
  assert.deepEqual(
    toggleTableColumnHidden(columns, ['driver', 'status'], 'vehicle'),
    ['driver', 'status'],
  );
...
test('reorders data columns without moving unknown ids', () => {
  assert.deepEqual(
    moveTableColumnOrder(['driver', 'status', 'vehicle'], 'vehicle', 'driver'),
    ['vehicle', 'driver', 'status'],
  );
  assert.deepEqual(
    moveTableColumnInOrder(['driver', 'status', 'vehicle'], 'status', -1),
    ['status', 'driver', 'vehicle'],
  );
  assert.deepEqual(
    moveTableColumnInOrder(['driver', 'status', 'vehicle'], 'driver', -1),
    ['driver', 'status', 'vehicle'],
  );
});

test('lists data columns then locked action rows for the customizer', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      { id: 'action', visible: true, hideable: false, reorderable: false, label: 'Action' },
    ],
  );
});
...
test('maps customizer rows to menu switch items', () => {
  assert.deepEqual(
    tableColumnCustomizerMenuItems(columns, ['status'], ['vehicle', 'driver']),
    [
      {
        label: 'Vehicle',
        value: 'vehicle',
