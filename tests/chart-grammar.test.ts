...
import {
  compileChartScene,
  findChartFocus,
  resolveDonutPadAngle,
} from '../src/wc/utils/chart-scene';
...
describe('chart grammar', () => {
  it('resolves field and accessor channels against ordinary rows', () => {
    ...
    assert.equal(
      channelValue(rows[1], 1, rows, (row, index) => row.value + index),
      8
    );
  });
  ...
  it('normalizes explicit percentage stacks without hiding the transform in a variant', () => {
    ...
    assert.deepEqual(
      normalized.map(row => row.proportion),
      [0.75, 0.25, 0]
    );
  });
  ...
  it('prepares stable pie fractions without introducing a chart-type component', () => {
    ...
    assert.deepEqual(
      slices.map(slice => [slice.key, slice.theta1, slice.theta2]),
      [
        ['a', 0, 0.75],
        ['b', 0.75, 1],
      ]
  ...
  it('bins raw values independently of rendered width and supports cumulative counts', () => {
    ...
    assert.deepEqual(
      cumulativeBins(bins).map(bin => bin.cumulative),
      [3, 5, 6]
    );
  });

  it('creates finite density samples from ordinary rows', () => {
    const density = densityX([{ value: 1 }, { value: 2 }, { value: 3 }], {
      value: 'value',
      samples: 12,
    });
...
describe('chart scene compiler', () => {
  ...
  it('preserves declaration order for layered marks', () => {
    ...
    assert.deepEqual(
      scene.nodes.map(node => node.markId),
      ['area', 'rule', 'line', 'dots', 'dots']
    );
    const dotNodes = scene.nodes.filter(node => node.markId === 'dots');
    assert.ok(
      dotNodes.every(
        node =>
          node.style.stroke === 'var(--color-background-primary)' &&
          node.style.strokeWidth === 'var(--dimension-stroke-width-012)'
      )
    );
    assert.ok(dotNodes.every(node => node.clipToPlot === false));
    assert.ok(
      scene.nodes
        .filter(node => node.markId !== 'dots')
        .every(node => node.clipToPlot === undefined)
    );
  });
  ...
  it('creates line gaps and omits invalid semantic points', () => {
    ...
    assert.deepEqual(
      scene.focusDotPointKeys,
      scene.points.map(point => point.sceneKey)
    );
    assert.equal(
      scene.nodes.some(node => node.type === 'circle'),
      false
    );
  });
  ...
  it('keeps explicit datum identity stable when rows reorder', () => {
    ...
    assert.deepEqual(
      make(rows)
        .points.map(point => point.sceneKey)
        .sort(),
      make([...rows].reverse())
        .points.map(point => point.sceneKey)
        .sort()
  ...
  it('lays out grouped bars side by side and stacks positive and negative values separately', () => {
    ...
    const grouped = compileChartScene(
      defineChart({
        marks: [
          barY(rows, {
            id: 'bars',
    ...
    const stacked = compileChartScene(
      defineChart({
        marks: [
          barY(rows, {
            id: 'bars',
  ...
  it('re-solves plot margins and thins measured ticks for narrow surfaces', () => {
    ...
    const wide = compileChartScene(definition, 900, 320, 'en', text => ({
      width: text.length * 8,
      height: 16,
    }));
    const narrow = compileChartScene(definition, 280, 220, 'en', text => ({
      width: text.length * 8,
      height: 16,
    }));
    assert.ok(
      narrow.xAxis.ticks.filter(tick => tick.labelVisible).length <
        wide.xAxis.ticks.filter(tick => tick.labelVisible).length
    );
  ...
  it('compiles pie and donut recipes through one polar scene', () => {
    ...
    assert.deepEqual(
      wide.points.map(point => point.value),
      [3, 1]
    );
    assert.equal(wide.center?.value, '4');
    assert.notDeepEqual(
      wide.points.map(point => point.x),
      narrow.points.map(point => point.x)
    );
    ...
    wide.arcHitRegions.forEach(region => {
      ...
      assert.equal(
        findChartFocus(wide.points, 'nearest', x, y, undefined, wide.arcHitRegions)?.primary
          .sceneKey,
        region.sceneKey
      );
    });
  ...
  it('maps heatmap intensity continuously from 25 to 100 percent data-intent opacity', () => {
    ...
    assert.deepEqual(
      cells.map(node => node.style.fillOpacity),
      [0.25, 1]
    );
  ...
  it('composes histogram rectangles, annotation bands, and box summaries as marks', () => {
    ...
    const boxScene = compileChartScene(
      defineChart({
    ...
    assert.ok(
      boxScene.nodes
        ...
        .every(
          node =>
            node.style.stroke === 'var(--color-data-intent-brand)' && node.style.strokeOpacity === 1
        )
    );
    assert.equal(
      boxScene.nodes.some(node => node.key.endsWith(':lower') || node.key.endsWith(':upper')),
      false
    );
  });
  ...
  it('builds polygon radar guides and linear radial series', () => {
    ...
    assert.deepEqual(
      scene.guides
        ...
        .map(node => (node.type === 'text' ? node.style.textAnchor : undefined)),
      ['middle', 'start', 'end']
    );
  });
  ...
  it('includes an implicit area baseline in the domain and keeps semantic points inside the plot', () => {
    const rows = [
      { id: 'a', x: 40, density: 0.01 },
      { id: 'b', x: 50, density: 0.04 },
      { id: 'c', x: 70, density: 0.015 },
    ];
    const scene = compileChartScene(
      defineChart({
    ...
    assert.ok(
      scene.points.every(point => point.y >= scene.plot.top && point.y <= scene.plot.bottom)
    );
  ...
  it('keeps guide candidates separate from visible labels and axis geometry', () => {
    const scene = compileChartScene(
      defineChart({
        marks: [
          barY([{ id: 1, category: 'A', value: 4 }], { key: 'id', x: 'category', y: 'value' }),
        ],
        x: { scale: scaleBand },
