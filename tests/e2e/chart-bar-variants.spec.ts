...
test.beforeEach(async ({ page }) => {
  await page.goto('/chart-bar-variants.html');
...
test('stacks raw series contiguously with only the visible top rounded', async ({ page }) => {
  ...
  const stacks = chart.locator('.chart-bar__stack');

  await expect(stacks).toHaveCount(2);
  await expect(chart.locator('.chart-bar__segment-shape')).toHaveCount(6);
  await expect(chart.locator('.chart-bar__segment-shape--rounded')).toHaveCount(2);
  await expect(chart.locator('.chart-bar__segment-separator')).toHaveCount(4);

  const stackGeometry = await stacks.evaluateAll(elements =>
    elements.map(stack =>
      Array.from(stack.querySelectorAll<SVGRectElement>('.chart-bar__segment-shape'))
        .map(segment => ({
...
test('normalizes every non-empty time bucket to a fixed 0–100% stack', async ({ page }) => {
  ...
  const axisLabels = chart.locator('.chart-bar__axis-label');
  const stacks = chart.locator('.chart-bar__stack');
  const segments = chart.locator('.chart-bar__segment-shape');
  const separators = chart.locator('.chart-bar__segment-separator');
  ...
  await expect(chart.locator('.chart-bar__segment-shape--rounded')).toHaveCount(0);
  await expect(chart.locator('.chart-bar__segment-square-base')).toHaveCount(0);
  ...
  const normalizedGeometry = await stacks.evaluateAll(elements =>
    elements.map(stack => {
      const segments = Array.from(
        stack.querySelectorAll<SVGRectElement>('.chart-bar__segment-shape')
      );
      return {
