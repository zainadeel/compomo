...
test.beforeEach(async ({ page }) => {
  await page.goto('/card-data-viz.html');
...
test('owns data-viz heading, actions, body, and token-based dimensions', async ({ page }) => {
  ...
  await expect(shell.locator('.card-data-viz__title')).toHaveText('Fuel trend');
  await expect(shell.locator('.card-data-viz__actions ds-button-unfilled')).toHaveCount(1);
  await expect(shell.locator('.card-data-viz__body #viz-content')).toHaveText('Visualization content');
...
test('donut variant owns its chart and legend composition', async ({ page }) => {
  const card = page.locator('#donut-card');
  await expect(card.locator('ds-card')).toHaveCount(0);
  await expect(card.locator('.card-data-viz__title')).toHaveText('Availability status');
  await expect(card.locator('.card-data-viz__chart ds-chart-donut')).toHaveCount(1);
  await expect(card.locator('.card-data-viz__legend ds-chart-legend')).toHaveCount(1);
});

test('line variant composes a fitting line chart with a static legend', async ({ page }) => {
  const card = page.locator('#line-card');
  const chartRegion = card.locator('.card-data-viz__chart');
  ...
  await expect(card.locator('.card-data-viz__title')).toHaveText('Fuel trend');
...
test('bar variant composes stacked bars with a static legend', async ({ page }) => {
  const card = page.locator('#bar-card');
  const chartRegion = card.locator('.card-data-viz__chart');
  const chart = card.locator('ds-chart-bar');
  const legend = card.locator('ds-chart-legend');
  const legendRows = legend.locator('.chart-legend__item');

  await expect(card.locator('.card-data-viz__title')).toHaveText('Vehicle activity');
  await expect(chartRegion.locator('ds-chart-bar')).toHaveCount(1);
  await expect(legend).toHaveJSProperty('highlightOnHover', false);
...
const readCartesianMetrics = async (
  ...
  chart.evaluate((element, chartKind) => {
    ...
    const separator =
      chartKind === 'stacked'
        ? element.querySelector<SVGLineElement>('.chart-bar__segment-separator')
        : null;
...
for (const chartCase of [
  ...
  { cardId: 'bar-card', selector: 'ds-chart-bar', kind: 'stacked' as const },
  { cardId: 'regular-bar-card', selector: 'ds-chart-bar', kind: 'bar' as const },
]) {
  test(`${chartCase.kind} ${chartCase.selector} reflows across card sizes without scaling visual primitives`, async ({
    page,
  }) => {
