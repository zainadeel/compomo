...
test('renders overlays without entry or exit animation delays', async ({ page }) => {
  ...
  await expect(page.locator('#banner .floating-toast')).toBeVisible();
  ...
  const animations = await page.evaluate(() => ({
    ...
    banner: getComputedStyle(document.querySelector('#banner .floating-toast')!).animationName,
  }));
  expect(animations).toEqual({ menu: 'none', modal: 'none', banner: 'none' });
  ...
  await expect(page.locator('#modal .modal-backdrop')).toHaveClass(/modal-backdrop--hidden/);

  await page.locator('#banner').getByRole('button', { name: 'Dismiss' }).click();
  await expect.poll(() => page.evaluate(() =>
    (window as typeof window & { __bannerDismissed: number }).__bannerDismissed
  )).toBe(1);
});
