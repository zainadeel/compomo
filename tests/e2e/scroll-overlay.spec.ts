...
test('keeps a 48px fade behind an inset md footer action at every scroll position', async ({
  ...
  const fadeWindow = async () =>
    frame.evaluate(element => {
      ...
      return {
        start: Number.parseFloat(style.getPropertyValue('--ds-scroll-edge-fade-window-start')),
    ...
    .poll(() =>
      frame.evaluate(element => ({
        ...
        fade: (() => {
          ...
          const end = Number.parseFloat(style.getPropertyValue('--ds-scroll-edge-fade-window-end'));
          return end - start;
        })(),
      }))
    )
