...
test('policy matrix covers every authored interactive component', () => {
  ...
  const interactiveComponents = fs.readdirSync(componentRoot).filter(component => {
...
test('component scale declarations remain limited to documented non-press animation owners', () => {
  ...
  const scaleOwners = cssFiles
    .filter(file =>
      /\b(?:scale|transform)\s*:[^;{}]*\bscale\(/.test(read(path.join('src/wc/components', file)))
    )
    .sort();

  assert.deepEqual(scaleOwners, ['Modal/Modal.css', 'SwatchPicker/SwatchPicker.css']);
});
