...
- Hover washes run only under `(hover: hover) and (pointer: fine)`. Touch browsers can latch `:hover` after a tap, so never add an unconditional hover selector to a shared interaction target. Touch normally keeps momentary `:active` press feedback. Persistent mobile shell chrome may instead own an explicit pointer press class when Safari must guarantee a visible quick-tap frame and deterministic release/cancel cleanup; scope that exception to coarse touch-only pointers.
- Shared focus-ring targets suppress native pointer/touch outlines and the WebKit tap highlight while retaining the authored `:focus-visible` ring for keyboard operation. They use `touch-action: manipulation` to prevent double-tap zoom on the control while preserving page panning and pinch zoom. Never disable page zoom through viewport metadata or `touch-action: none`.
...
- Button-like interaction targets use the default cursor across their complete hit area; actual links use the pointer cursor. Decorative interaction overlays and content layers inherit the owning target's cursor explicitly. Do not assign a different cursor to a descendant of an interactive target; cursor resolution follows hit testing and must remain stable as the pointer crosses its SVG, text, badge, and overlay layers.
