...
- Hover washes run only under `(hover: hover) and (pointer: fine)`. Touch browsers can latch `:hover` after a tap, so never add an unconditional hover selector to a shared interaction target. Touch keeps momentary `:active` press feedback.
