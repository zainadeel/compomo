...
function ensureRegistry() {
  ...
  if (!fs.existsSync(registryPath)) {
    if (!IS_SOURCE_CHECKOUT) {
      throw new Error(
        'The published @ds-mo/ui MCP registry snapshot is missing. Reinstall the package.'
      );
    }
...
function loadPatterns() {
  ...
  if (!fs.existsSync(PATTERN_MANIFEST)) {
    throw new Error(
      'The published @ds-mo/ui MCP pattern snapshot is missing. Reinstall the package.'
    );
  }
...
function createServer() {
  ...
  server.registerTool(
    'list_components',
    {
      ...
      description:
        'List all available components in the @ds-mo/ui design system library. Returns component names, descriptions, and required packages.',
      inputSchema: z.object({
        search: z
          .string()
          .optional()
          .describe('Optional text to filter components by name or description.'),
      }),
    ...
    async ({ search }) => {
      ...
      if (items.length === 0) {
        return {
          content: [
            { type: 'text', text: `No components found${search ? ` matching "${search}"` : ''}.` },
          ],
          structuredContent: {
            count: 0,
            query: search ?? null,
  ...
  server.registerTool(
    'get_component',
    {
      title: 'Get Component',
      description:
        'Get detailed information about a specific @ds-mo/ui component including props API, import statements, setup instructions, peer dependencies, and full source code for reference.',
      inputSchema: z.object({
        name: z
          .string()
          .describe(
            'Component name in kebab-case (e.g., "button", "tab-group", "toggle-button") or PascalCase (e.g., "Button", "TabGroup").'
          ),
      }),
  ...
  server.registerTool(
    'get_setup_guide',
    {
      ...
      description:
        'Get the full project setup guide for @ds-mo/ui — install commands, CSS imports, theme configuration, and design token architecture.',
  ...
  server.registerTool(
    'get_component_source',
    {
      ...
      description:
        'Get the full source code of a @ds-mo/ui component for reference. Use this to understand implementation patterns, not to copy the code into a project.',
      inputSchema: z.object({
  ...
  server.registerTool(
    'list_patterns',
    {
      title: 'List Patterns',
      description:
        'List supported @ds-mo/ui multi-component composition patterns with executable framework recipes.',
      inputSchema: z.object({
        search: z
          .string()
          .optional()
          .describe('Optional text to filter patterns by id, summary, or use case.'),
      }),
    ...
    async ({ search }) => {
      ...
      if (!patterns.length) {
        return {
          content: [
            {
              type: 'text',
              text: `No composition patterns found${search ? ` matching "${search}"` : ''}.`,
            },
          ],
          structuredContent: {
            count: 0,
      ...
      return {
        content: [
          {
            type: 'text',
            text: `# @ds-mo/ui — ${patterns.length} composition pattern${patterns.length === 1 ? '' : 's'}\n\n${formatPatternList(patterns)}`,
          },
        ],
        structuredContent: {
  ...
  server.registerTool(
    'get_pattern',
    {
      title: 'Get Pattern',
      description:
        'Get design intent, state ownership, accessibility requirements, and executable Custom Elements, React, Vue, or Angular recipes for an @ds-mo/ui composition.',
      inputSchema: z.object({
        name: z
          .string()
          .describe('Pattern id or name, for example "menu-trigger" or "pattern:menu-trigger".'),
        framework: z
          .enum(['customElements', 'react', 'vue', 'angular'])
          .optional()
          .describe('Return recipes only for this framework. Omit to return every implementation.'),
      }),
