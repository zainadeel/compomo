#!/usr/bin/env node
/**
 * Compile the /nav and /utils subpath exports to dist/lib (bundled ESM + d.ts).
 *
...
await build({
  entryPoints: ['src/wc/nav/index.ts', 'src/wc/utils/index.ts'],
  outdir: 'dist/lib',
  outbase: 'src/wc',
  bundle: true,
  format: 'esm',
...
console.log('  Built dist/lib exports (nav, utils): bundled ESM + declarations');
