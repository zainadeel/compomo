...
const preferDsText = {
  ...
  create(context) {
    ...
    if (context.settings.compomo?.mode !== 'consumer' && isTextAllowlisted(filename)) return {};
...
const preferDsIcon = {
  ...
  create(context) {
    const filename = context.filename ?? context.getFilename();
    if (context.settings.compomo?.mode !== 'consumer' && isIconAllowlisted(filename)) return {};
...
function isNeutralLayoutElement(nameNode) {
  return nameNode.type === 'JSXIdentifier' && (nameNode.name === 'span' || nameNode.name === 'div');
}
...
const preferDirectDsText = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Prefer ds-text as the layout box instead of wrapping it in a neutral span/div.',
    },
    schema: [],
  },
  create(context) {
...
function classExpressionHasSelectedFill(node) {
  ...
  if (node.type === 'LogicalExpression' || node.type === 'BinaryExpression') {
    return classExpressionHasSelectedFill(node.left) || classExpressionHasSelectedFill(node.right);
  }
